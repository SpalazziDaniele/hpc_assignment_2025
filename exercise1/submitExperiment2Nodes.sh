...
# on a 2 nodes. It also compiles the necessary C programs and prepares the workspace.
...
for DIM in "${dims[@]}"; do
    ...
    for P in "${procs[@]}"; do
        ...
        if [ $(( P % NODES )) -ne 0 ]; then
            TASKS_PER_NODE=$(( TASKS_PER_NODE + 1 ))
        fi

        sbatch --nodes="$NODES" --ntasks-per-node="$TASKS_PER_NODE" --time=$TIME systolicRunMultipleNodes.slurm "$DIM" "$P" "$repeats"
    done
done